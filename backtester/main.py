...
Examples:
  ...
  python main.py --optimize optimize.json --backend nevergrad --trials 200 --sampler CMA
    ...
    parser.add_argument("--setup-credentials", action="store_true", help="Setup Binance API credentials")
    parser.add_argument("--download-candles", nargs=4, metavar=("SYMBOL", "TIMEFRAME", "FROM", "TO"),
                        help="Download candles")
    parser.add_argument("--backtest", action="store_true", help="Interactive backtest editor")
    parser.add_argument("--experiments", type=Path, help="Run experiments from JSON config")
    parser.add_argument("--workers", type=int, default=4, help="Number of parallel workers")
    parser.add_argument("--optimize", type=Path, help="Optimize parameters from JSON config")
    parser.add_argument("--backend", choices=["optuna", "nevergrad"], default="optuna",
    ...
    if args.setup_credentials:
        ...
        cli.optimize(args.optimize, backend=args.backend, trials=args.trials,
