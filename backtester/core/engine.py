...
class Portfolio:
    ...
    def cancel_orders_for_position(self, position_id: int) -> None:
        ...
        self.pending_orders = [
            ...
            if po.parent_position_id != position_id
        ]
...
class BacktestEngine:
    ...
    def run(
        ...
        for candle in candles:
            ...
            for bot, portfolio, bot_id in zip(bots, portfolios, names):
                ...
                for raw in orders:
                    self._submit_order(
                        raw, candle, portfolio, result, bot_id=bot_id,
    ...
    def _circuit_breaker_tripped(
        ...
        peak = max(result.equity_curve + [cur_equity]) if result.equity_curve else cur_equity
        if peak <= 0:
            return False
    ...
    def _submit_order(
        ...
        if otype == OrderType.MARKET:
            if side == OrderSide.BUY:
                self._process_buy(
                    candle, portfolio, qty, result,
                    bot_id=bot_id, reason=o["reason"],
                    bracket=o,
                )
            else:
                self._process_sell(
        ...
        fill_reason = TriggerReason.LIMIT_FILL if otype == OrderType.LIMIT else (
            TriggerReason.STOP_LOSS if otype in (OrderType.STOP, OrderType.STOP_LIMIT,
                                                 OrderType.TRAILING_STOP)
            else TriggerReason.MARKET_ENTRY
        )
        po = PendingOrder(
            side=side, qty=qty, type=otype,
        ...
        for po in list(portfolio.pending_orders):
            ...
            if po.type == OrderType.LIMIT:
                if limit_triggers(po, candle.high, candle.low):
                    ...
                    self._fill_pending(po, fill_price, candle, portfolio, result, bot_id)
                    fired = True
            elif po.type in (OrderType.STOP, OrderType.STOP_LIMIT,
                             OrderType.TRAILING_STOP):
                if stop_triggers(po, candle.high, candle.low):
                    ...
                    self._fill_pending(po, po.stop_price, candle, portfolio, result, bot_id)
                    fired = True
            if not fired:
                survivors.append(po)
    ...
    def _fill_pending(
        ...
        reason = po.fill_reason.value if hasattr(po.fill_reason, "value") else str(po.fill_reason)
        if po.side == OrderSide.BUY:
            # Synthesize a candle with ref_price as close so slippage applies
            self._process_buy_at(ref_price, candle, portfolio, po.qty, result,
                                 bot_id=bot_id, reason=reason)
        else:
            self._process_sell_at(ref_price, candle, portfolio, po.qty, result,
                                  bot_id=bot_id, reason=reason,
    ...
    def _process_buy(
        ...
        new position.
        """
        self._process_buy_at(candle.close, candle, portfolio, qty, result,
                              bot_id=bot_id, reason=reason, bracket=bracket)
    ...
    def _attach_bracket_orders(
        ...
        if sl_price is not None:
            portfolio.pending_orders.append(PendingOrder(
                side=OrderSide.SELL, qty=pos.qty, type=OrderType.STOP,
                stop_price=sl_price,
        ...
        if tp_price is not None:
            portfolio.pending_orders.append(PendingOrder(
                side=OrderSide.SELL, qty=pos.qty, type=OrderType.LIMIT,
        ...
        if trail is not None and trail > 0:
            ...
            portfolio.pending_orders.append(PendingOrder(
                side=OrderSide.SELL, qty=pos.qty, type=OrderType.TRAILING_STOP,
                stop_price=stop,
                trail_pct=trail,
    ...
    def _process_sell(
        ...
        self._process_sell_at(candle.close, candle, portfolio, qty, result,
                               bot_id=bot_id, reason=reason)
    ...
    def _process_sell_at(
        ...
        if parent_position_id is not None:
            targets = [p for p in portfolio.positions if p.position_id == parent_position_id]
        else:
