...
class OptimizationConfig:
    ...
    search_space: dict[
        str, dict[str, Any]
    ]  # param_name → {type, low, high, step, choices}
...
class Objective:
    ...
    def evaluate(self, params: dict[str, Any]) -> OptimizationResult:
        ...
        train_result = engine.run(
            bot, train_candles, self.cfg.symbol, self.cfg.timeframe
        )
        ...
        if val_candles:
            ...
            val_result = engine.run(
                bot_val, val_candles, self.cfg.symbol, self.cfg.timeframe
            )
